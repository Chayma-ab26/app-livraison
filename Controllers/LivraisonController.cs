...
namespace application_Livraison.Controllers
{
    ...
    public class LivraisonController : ControllerBase
    {
        ...
        public LivraisonController(ApplicationDbContext context, NotificationService notificationService)
        ...
        public async Task<ActionResult<Livraison>> CreateLivraison([FromBody] LivraisonCreateDto livraisonDto)
        {
            ...
            var chauffeur = await _context.Utilisateurs.FindAsync(livraison.ChauffeurId);
            if (chauffeur != null)
            {
                await _notificationService.SendAssignmentNotificationAsync(chauffeur, $"Nouvelle livraison assignée : {livraison.Client} à livrer.");
            }
        ...
        *//*
        ...
        public async Task<IActionResult> AssignChauffeur(int id, [FromBody] int chauffeurId)
        {
            ...
            var chauffeur = await _context.Utilisateurs.FindAsync(chauffeurId);
            if (chauffeur != null)
            {
                await _notificationService.SendAssignmentNotificationAsync(chauffeur, $"Vous avez une nouvelle livraison réassignée (ID: {livraison.Id}).");
            }
